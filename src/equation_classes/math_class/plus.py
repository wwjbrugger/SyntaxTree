...
def use_more_concrete_units(args, units_0, units_1, str_operator):
    result_unit = []
    for i,_ in enumerate(units_0):
        u_0 = units_0[i]
        u_1 = units_1[i]
        if u_0 == u_1:
            result_unit.append(u_0)
        elif isinstance(u_0, str) and isinstance(u_1, str):
            result_unit.append(f"{u_0} {str_operator} {u_1}")
        elif isinstance(u_0, float) and isinstance(u_1, float):
            if not u_0 == u_1:
                raise UnitError()
            else:
                result_unit.append(u_0)
        elif isinstance(u_0, float):
            result_unit.append(u_0)
        elif isinstance(u_1, float):
            result_unit.append(u_1)
    return result_unit
...
def propagate_units_same(node, call_node_id,dataset, kwargs, changed=False):
    ...
    changed |= node.list_children[1].units.update(use_more_concrete_units(
        node.tree.args,
        units_c0, units_self,
        '=='
    )
